...
molecular gallery and magic suite (`n19_suite.json`), the GFlowNet guard (`gflow.json`) and the
...
| command | result |
|---|---|
| `python src/verify.py` | **PASS** — 831 checks, 0 failures, 9 846 numeric assertions, 5 registered xfail, 0 xpass, 2 skips *(2026-09-19, after section (10b) and the `ladder_L4` open defect arrived with the C3 frontier deposit; the 807 / 7 454 / 4 reading of earlier the same day is superseded)* |
