...
# REAL hardware: 8-seed recovery MEAN per step + std per step (the honest per-point error bar)
mean=n['dE_hist_mean']; std=n['dE_hist_std']
hwc=" ".join(f"({i},{m:.3f})+-(0,{s:.3f})" for i,(m,s) in enumerate(zip(mean,std)))
dEm=n['dE_mean_mHa']; dEs=n['dE_std_mHa']         # converged 0.59 +- 0.14
s0m,s0s=mean[0],std[0]                             # step-0 point (starts with the noiseless curve)
...
\begin{axis}[name=axb,hpanel,at={(axa.south east)},anchor=south west,xshift=1.4cm,
  ...
  % (job da125f2ein7c73bcsqs0, no new QPU); error bar at each step = std across the 8 seeds.
...
print(f'wrote fig_hardware_hero_frag.tex | 8-seed mean+-std per step: '
