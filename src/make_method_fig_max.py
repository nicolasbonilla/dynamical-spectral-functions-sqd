...
\nextgroupplot[
  ...
  \node[anchor=north east, align=right, inkPrim] at (rel axis cs:0.985,0.56)
     {rel-$L_1=__L1F__$\\[-1pt] $\int_W\! A_{\mathcal S}=__WSR__$};
...
\end{tikzpicture}
"""
# The panel (a) annotation quotes the WINDOWED INTEGRAL of the reconstruction, which
# data/method_max.json deposits directly; 'sumrule' (0.500) is the analytic identity
# and is NOT what the panel shows.  rho and fit stay computed above because the
# convergence numbers are still reported in prose; they are simply not drawn.
wsr = d['sum_rule']['sum_rule_reconstructed_integrated']
tsh = '%.1f' % (shots_total / 10 ** 4) + r'\times10^{4}'
L=(L.replace('__WSR__',f'{wsr:.4f}').replace('__L1F__',f'{l1_complete:.3f}')
    .replace('__TSH__',tsh)
    .replace('__KS__',f'{ks}').replace('__LO__',lo).replace('__HI__',hi).replace('__PTS__',pts))
