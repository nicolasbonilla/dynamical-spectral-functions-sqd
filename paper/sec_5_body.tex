...
$L=8$ on, the coordinate support of the Krylov space $\mathcal K(H,\phi)$ is the whole sector, so
...
The result is negative and we state it first. At every published fraction the leakage branch of the
bound exceeds the trivial bound $\lVert\phi\rVert^{2}(1+w_S)$: by $3.35$ at $L=6$
($\mathrm{FR}=0.80$), $3.65$ at $L=8$ ($0.56$), $3.62$ at $L=10$ ($0.40$) and $4.86$ at $L=12$
($0.18$) at $\eta=0.15\,t$, and by $2.76$, $3.02$, $2.99$ and $4.00$ at $\eta=0.18\,t$. At $L=8$ and
$L=12$ these are the published fractions exactly; at $L=6$ and $L=10$ the nearest grid points are
$0.80$ and $0.40$ against published $0.82$ and $0.36$, and at $L=10$ the published fraction is the
worse of the two.

The largest case carries the evidence. At $L=12$, $\mathrm{FR}=0.18$, $\eta=0.18\,t$---that is,
$\lvert S\rvert=131\,725$ of $731\,808$ determinants---the captured weight is $w_S=0.998804$, so
Eq.~\eqref{eq:tau} asks for $\widehat\Lambda_S/\eta<\tau(w_S)=0.965$ and the measured value is $3.96$,
putting the leakage branch at $7.986$ against a trivial branch of $1.9988$. That is converged in depth
three ways: $\Lambda_{\rm in}/\Lambda_{\rm out}=3.0\times10^{-3}$; $\Lambda_{\rm out}$ changes by
$7.5\times10^{-7}$ between depths $250$ and $300$; and an independently written re-implementation moves
it by $8.6\times10^{-5}$ between depths $100$ and $300$. Since $\Lambda$ \emph{decreases} with depth,
...
The same fraction gives $7.51$, $4.86$ and $2.81$ at $\eta=0.10$, $0.15$, $0.25\,t$; and over the $606$
...
independently. What fails in this section is not the reconstruction and not that curve; it is the word
...
three, uniformly over $L=6$--$12$ and $\eta=0.10$--$0.25\,t$, and it is computed from $H$, $S$, $\phi$
and $\eta$ alone, without the answer. The objection to anticipate is that this is
...
Three limits are declared with it. The crossing is one point per cell, so the tracker locates one error
...
$w_S=0.998804$ and $\tau=0.9651$, and every subspace with $w_S\ge0.995$ has $\tau\ge0.9280$. A user
...
magnitude smaller; containment, not spanning, is what $\Lambda_S=0$ requires.) At $L=8$ the leakage is
therefore strictly positive for \emph{every} proper coordinate subspace, and no certificate of leakage
type, in this paper or another, can be non-vacuous there at low fraction in the determinant basis. That
is a property of the basis in which the device measures, not of the bound and not of the ranking. We
measured it at two sizes and state it for two: at $L=6$ four determinants escape the support, and
whether the $L=8$ behaviour persists at $L=10$ and $L=12$ is open. It is the single cheapest
measurement that would close this section, and we have not made it. Nor is the
