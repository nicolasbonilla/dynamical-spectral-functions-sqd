...
Section~\ref{sec:retraction} rules out an error bound the subspace construction was supposed to
satisfy. It rules out nothing about the construction itself. One structural guarantee does hold, and
...
$x\mapsto\mathcal L_\eta(\omega-x)$ together with a tail estimate, and moment exactness supplies
neither. The natural tool, a Bernstein-ellipse bound, is also where the resolution enters, through the
poles of the kernel at $x=\omega\pm i\eta$ (App.~\ref{app:moments:conj}); and neither $C(\eta)$ nor
...
One figure of this work is read against that conjecture and not against a theorem. Panel~(b) of
Fig.~\ref{fig:method} fits a geometric collapse $\text{rel-}L_1\sim\rho^{-K}$ with $\rho\approx1.7$,
and a fit of that shape is easily reported as ``precisely the geometric convergence guaranteed by the
