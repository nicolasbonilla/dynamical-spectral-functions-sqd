...
A sample-based reconstruction is paid for on three axes, not one: the determinant support
the sampler must populate, the resolution $\eta$ at which the response is asked for, and
the shot budget $T$ that populates the support. The previous version of this work priced the first
two, and quoted the second as though it were a property of the model. It is not. This section reports
what each axis costs, states which of the three exponents belongs to the Hubbard chain and which
belongs to the question we put to it, and adds the axis that was missing.

\paragraph{One symbol, three counts.} Because the same letter carries three different objects in this
paper, we fix the notation before using it. $|\mathcal S|_\eta$, the subject of this section, is
\emph{dynamical}: the number of determinants needed for the reconstruction of $A(\omega)$ to reach
relative $L_1<\mathrm{thr}$ at resolution $\eta$ and recursion depth $n_\ell$, so it depends on all
three of $\mathrm{thr}$, $\eta$ and $n_\ell$ (\texttt{scaling\_lanczos\_mf.py}, \texttt{stage\_frac}:
$\mathrm{thr}=0.05$, $\eta=0.15\,t$, $n_\ell=150$). $|\mathcal S|_{\varepsilon_w}$, the subject of
Sec.~\ref{sec:nogo}, is \emph{static}: the number of determinants carrying a fixed fraction
$1-\varepsilon_w$ of the weight of the \emph{ground state}, with no dynamics, no resolution and no
threshold on a spectral error. And $S$, the subject of Theorem~\ref{thm:leakage}, is neither: it is
whatever set of bitstrings the sampler actually returned. The three coincide only by accident. Where
this section writes $|\mathcal S|_\eta$ it means the first, and no statement here transfers to the
other two without saying so.
...
(Table~\ref{tab:fracgrid}). There is one resolution-dependent exponent in this analysis, and the
previous version reported it twice.
...
The headline of the previous version---that the fraction needed for a fixed spectral accuracy falls
from $0.92$ at $L=4$ to $0.08$ at $L=14$, as $e^{-0.25\,L}$---is tunable, and we withdraw it. Over the
decade of $\eta$ the fraction exponent moves by a factor $2.2$, from $-0.1266$ at $\eta=0.05\,t$
to $-0.2743$ at $\eta=0.50\,t$. At $\eta=0.50\,t$ the fraction at $L=12$ is already $0.10$, below the
value the previous version quoted for $L=14$: any headline of this form is reachable by choosing the
broadening.
...
The mechanism that makes the fraction move with $\eta$ was already stated, without derivation, in
v1--v2, and Sec.~\ref{sec:moments} restates it as Conjecture~\ref{conj:geom} rather than as a theorem:
...
The replacement is therefore stated as a declared window rather than as a number. The fraction is
reported only for $\eta\ge0.15\,t$ at $n_\ell=150$; the full $\mathrm{frac}(L,\eta)$ grid of
Table~\ref{tab:fracgrid} is published with it, so that the resolution dependence is visible rather
than absorbed into a single exponent; and the quantitative scaling claim of this work is carried by
the support, Eq.~\eqref{eq:supportexp}, whose exponent is quoted with its fit window attached.

\subsection{The third price: shots}

The two axes the previous version priced are the fraction and the absolute support, and the caption of
Fig.~\ref{fig:scaling} called that pair the honest two-sided picture. There is a third, it is the one
that binds, and---unlike the other two---it had no number at all.
...
Precision about the label matters more here than the retraction. The ranking that produced the
...
But it is also not a finite-shot reconstruction, and describing it as \emph{sampled} without a shot
count was wrong. The published curve is the $T\to\infty$ limit of the declared protocol, and it is
