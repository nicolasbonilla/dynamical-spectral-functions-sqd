...
$5\times10^{4}$ computational-basis shots pooled over the $t=0$ reference and $K=7$
real-time-evolution circuits, under TREX readout twirling, Pauli twirling and dynamical decoupling and
...
on trapped ions against scattering data~\cite{quantinuum_dsf2026,greenediniz2024}---so our device
record is the smallest in its comparison class and is not offered as a scale result. What is new in it
...
shots: the $5\times10^{4}$ spent are about twice the $(2.43\pm0.34)\times10^{4}$ that
Table~\ref{tab:shots} measures for one orbital seed and one branch at $L=6$, so complete coverage of a
$300$-dimensional sector is what the budget predicts rather than a surprise.\footnote{The device run
used $K=7$ evolution circuits and the replication of Table~\ref{tab:shots} the repository protocol,
$K=18$ with $19$ snapshots; this is a consistency statement between two budgets, not an identity.} The
first size at which a device run would test the reconstruction rather than the pipeline is $L=8$:
sector $3\,920$, and $(2.70\pm0.13)\times10^{5}$ shots per orbital seed and branch, a factor $5.4$
above what was spent here.
...
Dynamics in one dimension is where classical methods arrived first, and the framing sentence is
withdrawn. Within the region this work measures---one-dimensional Hubbard rings at $L\le12$, the
...
certificate at small sampled fraction requires a different working basis, not a better ranking within
this one.
...
Theorem~\ref{thm:leakage} is vacuous at every fraction this work publishes, by factors of $2.8$ to
$4.9$ at the four sizes measured, so the reconstructions reported here are at present uncertified and
...
measured at four sizes up to a sector of dimension $731\,808$ and its crossing calibrated to a true
...
%    807 / 7454 / 4   -- python src/verify.py on this tree, re-measured at freeze time by
...
the deposited artefacts against it---$807$ checks over $7\,454$ numeric assertions, exiting non-zero on
...
and four known open defects are printed by name on every run.
