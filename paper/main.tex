...
%  That is why 33 of the 17+16 floats are starred -- all of them: it is the assignment
...
%  comment said '35 of the 19+16' until 2026-09-19, two figures after the fact.)
% =====================================================================================
...
\documentclass[
  ...
  floatfix          % REVTeX's own fix for deferred double-column floats (all 33)
]{revtex4-2}
...
% All 33 floats are double-column, and a figure* can only reach the top of a
...
% 33 floats are taller than that on their own, so each of them qualified to fill a
...
% text, exactly as before.  Measured on this tree: 13 text-free pages -> 2, 59 -> 57.
...
can do this: an analytic counterexample forces the constant of any such bound to its trivial value, and
none of $606$ subspaces violates the bound proved here. On Hubbard sectors to dimension $731\,808$ ours
is worse than the trivial bound at every sampled fraction, at all four sizes, by $2.8$--$4.9$, converged
in depth at $L=12$, so those reconstructions are uncertified, though independent code recovers those
fractions to $7\%$. The obstruction is structural: at $L=8$ the probe's Krylov space touches every
determinant of the sector. What survives is a calibration: over sixteen $(L,\eta)$ cells spanning five
decades of true error, the bound crosses the trivial one at a true relative error of
