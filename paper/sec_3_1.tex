...
%  This file defines only sec:retraction, eq:retracted, eq:exclusion, eq:mu2.
...
\subsection{Retraction of Theorem~1(iii): the captured weight is not the controlling magnitude}
\label{sec:retraction}

Versions~1 and~2 of this preprint (arXiv:2608.16436) carried, as Theorem~1(iii), an error bound indexed
on the captured weight alone,
\begin{equation}
\lVert A-A_S\rVert_{1}\;\le\;2\lVert\phi\rVert^{2}(1-w_S)\;+\;C\,\rho^{-K_S},
\label{eq:retracted}
\end{equation}
$w_S=\lVert P_S\phi\rVert^{2}/\lVert\phi\rVert^{2}$, $K_S$ the largest Krylov order contained in
$\operatorname{span}(S)$ (we set $K_S=-1$ when $\phi\notin\operatorname{span}(S)$). The main text invoked
it as the certificate controlling the reconstruction once $w_S\to1$, the discussion as the claim that
the error ``is bounded by the captured spectral weight''. Equation~\eqref{eq:retracted} is false, and we
retract it together with both invocations; Theorem~\ref{thm:leakage} replaces it, keeping the one
inequality of the published argument that was correct, with the direction it always had.
...
\eqref{eq:retracted} requires $C\ge2\lVert\phi\rVert^{2}$: false if $C$ is a constant, vacuous if it may
...
the family \eqref{eq:retracted}, and Theorem~\ref{thm:leakage} keeps it explicitly, as the first branch
of its minimum.
...
retracted bound fails on $468$, and restricted to the regime the paper invoked, $w_S\ge0.99$, it fails
...
$0.95$: the retracted bound fails worst exactly where the reconstruction is best, which is the opposite
...
factor $2.3$, so only its order of magnitude is quoted. None of this touches the retraction: the
...
weight the subspace \emph{keeps} is nonetheless moved in $\omega$. The published sketch elided exactly
this. Having correctly noted $\int A=\lVert\phi\rVert^{2}$ and $\int A_S=\lVert P_S\phi\rVert^{2}$, it
bounded the $L_1$ discrepancy by twice the missing weight --- a step legitimate when $[P_S,H]=0$, where
...
across the same $239$ subspaces on which the upper form fails. It survives the retraction as the
...
\eqref{eq:retracted} are therefore never both informative: either $w_S=1$ and the whole bound is the
...
that v1--v2 used there (Fig.~\ref{fig:scaling}; the coarser replication grid of
Table~\ref{tab:fracgrid} reads $0.180$ at $L=12$ for the same operating point). From $L=8$ on, every
operating point v1--v2 published lies below its own floor, so $K_S=-1$ by counting alone, with no
...
file of the reproduction repository: the retracted bound was quoted, never applied, which is how a
statement false by counting reached a published version. Theorem~\ref{thm:leakage} carries no such
...
Two measurements at the same, exactly unit, captured weight show that \eqref{eq:retracted} was indexed
