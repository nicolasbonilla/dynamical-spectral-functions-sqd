...
\caption{\textbf{The published bound of Theorem~1(iii) fails on exactly the subspaces the paper
relies on; the replacement does not.}
For each subspace $\mathcal S$ we plot the measured error $\|A-A_{\mathcal S}\|_1$ divided by the
bound, so every point above the dashed line at $1$ is a violation. Orange: the published weight
bound $2\|\varphi\|^{2}(1-w_{\mathcal S})$, split by captured weight (filled, $w_{\mathcal S}\ge
0.99$; open, $w_{\mathcal S}<0.99$). Blue: Theorem~B as stated, $\min\{\|\varphi\|^{2}(1+w_{\mathcal
...
weight-ordered or structured selections. \emph{The published bound is violated on $468$ of the
$606$.} On $50$ of them it is identically zero: those subspaces carry the whole weight to double
...
at the published fraction $0.85$. Those $64$ rows sit at only four abscissae, so they are drawn as a min--median--max whisker at each one rather than as $64$ overlapping markers (medians $18.5, 23.6, 26.5, 29.9$ at fractions $0.50, 0.70, 0.85, 0.95$); each of them is also plotted individually inside the filled orange series, which is where their scatter can be read. Theorem~B is violated on none of the $606$; its ratios span
...
figure shows that Theorem~B \emph{holds}, not that it is tight. In (b) the $\eta$ axis is not a sweep
