...
them. Its pooled coefficient is withdrawn together with $\mathcal F_1$'s: pooling six deterministic
five-point sweeps destroys within-stratum structure whichever way the result falls, and it fell in our
favour on one of the two.
...
Spearman $\rho=+0.722$), with the geminal witness $\chi{=}2$, $|\mathcal S|{=}2^{K}$ the
...
\textbf{(b)}~One-body magic does not ($\rho=+0.599$ over the same rows): the Hubbard branch runs
