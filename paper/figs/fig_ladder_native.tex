...
\begin{tikzpicture}
\begin{groupplot}[group style={group size=2 by 1, horizontal sep=1.75cm},
  width=7.15cm, height=6.0cm, paperaxis,
...
\nextgroupplot[title={(a)~minimal bond dimension $\chi$},
  xlabel={qubits $\;2L$}, ylabel={$\chi$ (min.\ MPS bond dim.)}, ymin=0, ymax=62,
...
\node[ldInk,font=\scriptsize,anchor=north east] at (axis cs:21.0,55.0) {$\mathcal F_1$ matched};
\node[ldInk,font=\scriptsize,anchor=west] at (axis cs:16.55,27.0) {$\times4.8$};
% ---------- (b) required (N+-1) fraction ----------
\nextgroupplot[title={(b)~response-targeted $(N{\pm}1)$ fraction},
...
\addplot[ldLad,line width=1.5pt,mark=*,mark size=3pt,mark options={fill=ldLad,draw=white,line width=0.8pt}]
  coordinates {(12,0.807)(16,0.626)};
\addplot[ldChain,line width=1.5pt,mark=square*,mark size=2.8pt,mark options={fill=ldChain,draw=white,line width=0.8pt}]
  coordinates {(12,0.717)(16,0.426)};
