...
```
.
├── src/                       # 60 computation & figure scripts (run from repo root as `python src/X.py`)
...
|---|---|---|
| **The guardian** | ✅ **PASS** — 831 checks, 0 failures, 9 846 numeric assertions, 5 xfail, 0 xpass, 2 skips, ~9 s *(2026-09-19, after the C3 frontier deposit added section (10b))* | `python src/verify.py` (or `make verify`) |
